...
- [x] Initialize the project with `React + TypeScript + Vite`.
- [x] Add a minimal app shell that renders a plain `Hello World` page.
- [x] Add a basic global stylesheet and confirm the app starts locally.
- [x] Create a simple `README.md` with local setup instructions.
- [x] Add `.env.example` with placeholder environment variables and no secrets.
- [x] Add a basic security note in the docs covering secret handling and client/server boundaries.
- [x] Set up code formatting and linting.
- [x] Set up CI/CD to run install, lint, and build on every push and pull request.
